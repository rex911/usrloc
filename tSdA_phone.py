...
def test_tSdA(finetune_lr=0.03, pretraining_epochs=25,
             pretrain_lr=0.1, training_epochs=1000,
             dataset='phone.data.gz', batch_size=32, state_wise=True):
    """
    ...
    f = gzip.open('sda_phone.model', 'wb')
    cPickle.dump(sda, f)
    f.close()
    end_time = time.clock()
    ...
    f = gzip.open('sda_phone.model', 'rb')
    sda = cPickle.load(f)
    f.close()
    end_time = time.clock()
...
if __name__ == '__main__':
    ...
    lr = [1, 0.3, 0.1, 0.01, 0.003, 0.001, 0.0003, 0.0001]
    for i in lr:
        print 'lr =', i
        test_tSdA(finetune_lr=i)
        test_tSdA(finetune_lr=i, state_wise=False)
