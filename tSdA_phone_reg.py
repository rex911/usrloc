    ...
    for i in users:
        ...
        if temp.find('country') is not None:
            if temp.find('state') is not None:
                if temp.find('country').text == 'United States of America' and temp.find('state').text in regions:
                    X_raw_t.append(i.text)
                    y_t.append([float(j) for j in i.geotag.split(' ')])

    X_raw, y = X_raw_t, y_t
    leny = len(y)
    print 'Size of dataset:', leny
...
def test_tSdA(finetune_lr_0=0.001, t=30, pretraining_epochs=25,
             pretrain_lr=0.10, training_epochs=1000,
             dataset='phone.data.gz', batch_size=32):
    """
    ...
    f = gzip.open('sda_phone_reg.model', 'wb')
    cPickle.dump(sda, f)
    f.close()
    end_time = time.clock()
    ...
    f = gzip.open('sda_phone_reg.model', 'rb')
    sda = cPickle.load(f)
    f.close()
    end_time = time.clock()
    ...
    patience = 1024 * n_train_batches  # look as this many examples regardless
...
# save the fine tuned model
        if epoch == 309:
            f = gzip.open('sda_phone_reg_ftd.model', 'wb')
            cPickle.dump(sda, f)
            f.close()
            break
##########################
